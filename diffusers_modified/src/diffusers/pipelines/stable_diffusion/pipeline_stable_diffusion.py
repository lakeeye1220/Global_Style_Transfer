...
class CLIPFeatureExtractor:
    def __init__(self, model, layers):
        ...
        def save_output(module, inp, out, idx=None):
            if idx in self.layers:
                hidden = out[0] if isinstance(out, tuple) else out
                self.features.append((idx, hidden))#.detach().cpu()))

        # vision transformer blocks
        for idx, blk in enumerate(self.model.vision_model.encoder.layers):
            if idx in self.layers:
                h = blk.register_forward_hook(
                    lambda m, i, o, idx=idx: save_output(m, i, o, idx)
                )
                self.hooks.append(h)

    def __call__(self, x):
        self.features.clear()
        _ = self.model.get_image_features(x)  # vision forward 호출
        self.features.sort(key=lambda t: t[0])
        return [f for _, f in self.features]
...
class StableDiffusionPipelineGuide(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin, FromSingleFileMixin):
    """
    Pipeline for text-to-image generation using Stable Diffusion.
    ...
    def pred_x0_from_eps(self,latents, noise_pred, t, scheduler):
        # alphas_cumprod[t]: shape [ ], 가져와서 [B,1,1,1]로
        alpha_prod_t = scheduler.alphas_cumprod[t].to(latents.device)          # scalar
        alpha_prod_t = alpha_prod_t.view(1, 1, 1, 1)
        sqrt_alpha = torch.sqrt(alpha_prod_t)
        sqrt_one_minus_alpha = torch.sqrt(1.0 - alpha_prod_t)
        # x0 = (x_t - sqrt(1 - α_t) * ε) / sqrt(α_t)
        x0 = (latents - sqrt_one_minus_alpha * noise_pred) / (sqrt_alpha + 1e-8)
        return x0
    '''
    def get_features(self, x, model, layers):
        ...
        for name, layer in enumerate(model.children()): # 0, conv
            ...
            features.append(x)
        print("feuature length : ",len(features))
        return features
    '''
    def get_content_loss(self, pred_features, target_features):
    ...
    def __call__(
        ...
        # 1. Check inputs. Raise error if not corrects
        self.check_inputs(
            prompt, height, width, callback_steps, negative_prompt, prompt_embeds, negative_prompt_embeds
        )
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                with torch.enable_grad():
                    ...
                    x0 = self.pred_x0_from_eps(latents, noise_pred, t, self.scheduler)
                    x0_scale= 1 / 0.18215 * x0
                    image = self.vae.decode(x0_scale).sample
                    
                    #image = self.vae.decode(guided_latents).sample
                    image = (image / 2 + 0.5).clamp(0, 1)

                    guided_latents_scale = 1 / 0.18215 * guided_latents
                    x_t = self.vae.decode(guided_latents_scale).sample
