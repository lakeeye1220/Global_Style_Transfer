...
def main():
    ...
    #unet.controlnet.fc1.weight.data *=1.25
    ...
    directory = "../vangogh2photo/trainB/"
    ...
    for j in range(args.num_test_samples):
        ...
        for prompt, concept in zip(*dataloader):
            ...
            if concept is not None:
                #DDIM inversion x clip guidance, recon guidance
                '''
                result_img =predict_cond(args,model=model_guide, content_latent = content_latent, content_img = content_img, prompt=prompt, seed=seed, condition=concept, img_size=args.resolution,
                                            ...
                                            negative_prompt=args.negative_prompt, content_logits = content_probs, clip_guide = False, classifier_guide=True, recon_guide=False
                                        )
                print("DDIM inversion O, classifier embed_guidance img sim: ",img_to_img_similarity(content_img, result_img))
                print("DDIM inversion O, classifier embed_guidance txt sim: ",txt_to_img_similarity(prompt, result_img))
                images.append(result_img)
                
                result_img =predict_cond(args,model=model_guide, content_latent = random_latent, content_img = content_img, prompt=prompt, seed=seed, condition=concept, img_size=args.resolution,
                                            num_inference_steps=args.num_inference_steps,
                                            negative_prompt=args.negative_prompt, content_logits = content_probs, clip_guide = False, classifier_guide = False, recon_guide=True
                                        )
                print("DDIM inversion x, recon guidance img sim: ",img_to_img_similarity(content_img, result_img))
                print("DDIM inversion x, recon guidance txt sim: ",txt_to_img_similarity(prompt, result_img))
                images.append(result_img)
                ...
                print("DDIM inversion O, recon_guidance img sim: ",img_to_img_similarity(content_img, result_img))
                print("DDIM inversion O, recon_guidance txt sim: ",txt_to_img_similarity(prompt, result_img))
                images.append(result_img)
                '''
                result_img =predict_cond(args,model=model_guide, content_latent = content_latent, content_img = content_img, prompt=prompt, seed=seed, condition=concept, img_size=args.resolution,
                                            num_inference_steps=args.num_inference_steps,
                ...
                #                            negative_prompt=args.negative_prompt, content_logits = content_logits, clip_guide = True, classifier_guide = False, recon_guide=False
                #                      )
                #print("DDIM inversion O, content_guidance+NST guidance img sim: ",img_to_img_similarity(content_img, result_img))
                #print("DDIM inversion O, content_guidance+NST guidance txt sim: ",txt_to_img_similarity(prompt, result_img))
                #images.append(result_img)
                '''
                result_img =predict_cond(args,model=model_guide, content_latent = random_latent, content_img = content_img, prompt=prompt, seed=seed, condition=concept, img_size=args.resolution,
                                            num_inference_steps=args.num_inference_steps,
                                            negative_prompt=args.negative_prompt, content_logits = content_logits, clip_guide = True, classifier_guide = False, recon_guide=True
                                      )
                print("DDIM inversion X, content_guidance img sim: ",img_to_img_similarity(content_img, result_img))
                print("DDIM inversion X, content_guidance txt sim: ",txt_to_img_similarity(prompt, result_img))
                images.append(result_img)
                ...
                print("DDIM inversion O, content_guidance, recon img sim: ",img_to_img_similarity(content_img, result_img))
                print("DDIM inversion O, content_guidance, recon txt sim: ",txt_to_img_similarity(prompt, result_img))
                images.append(result_img)
                #h-space (0) content clip guidance recon None!!!!
                
                # h-space guidance and clit guidance+recon (ours)
                result_img = predict_cond(args,model=model_guide, content_latent = content_latent,content_img = content_img, prompt=prompt, seed=seed, condition=concept, img_size=args.resolution,
                                            num_inference_steps=args.num_inference_steps,
                                            negative_prompt=args.negative_prompt, content_logits = content_probs, clip_guide = True, classifier_guide = True, recon_guide=True
                                        )
                images.append(result_img)
                print("Ours img sim (3o): ",img_to_img_similarity(content_img, result_img))
                print("Ours txt sim (3o): ",txt_to_img_similarity(prompt, result_img))
                '''
            #else:
